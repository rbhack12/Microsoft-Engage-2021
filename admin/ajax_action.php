...
require_once('../user/class/class.phpmailer.php');
...
if(isset($_POST['page']))
{
	if($_POST['page'] == 'register')
	{
		if($_POST['action'] == 'email_check')
		{
			...
			WHERE mail_address_admin = '".trim($_POST["email"])."'
			";
		...
		if($_POST['action'] == 'register')
		{
			...
			$receiver_email = $_POST['mail_address_admin'];

			$exam->data = array(
				':mail_address_admin'		=>	$receiver_email,
			...
			(mail_address_admin, password_admin, admin_verfication_code, admin_type, admin_created_on, email_verified) 
			VALUES 
			(:mail_address_admin, :password_admin, :admin_verfication_code, :admin_type, :admin_created_on, :email_verified)
			";
	...
	if($_POST['page'] == 'login')
	{
		...
		{
			$exam->data = array(
				':mail_address_admin'	=>	$_POST['mail_address_admin']
			);

			$exam->query = "
			SELECT * FROM admin_table 
			WHERE mail_address_admin = :mail_address_admin
			";
			...
			if($total_row > 0)
			{
				...
				foreach($result as $row)
				{
					...
					{
						if(password_verify($_POST['password_admin'], $row['password_admin']))
						{
	...
	if($_POST['page'] == 'exam')
	{
		...
		if($_POST['action'] == 'Edit')
		{
			...
			SET marks_per_right_answer = :marks_per_right_answer, marks_per_wrong_answer = :marks_per_wrong_answer, total_question = :total_question, online_exam_duration = :online_exam_duration, online_exam_datetime = :online_exam_datetime, online_exam_title = :online_exam_title
			...
			$output = array(
				'success'	=>	'Details of the exam is changed'
			);
	...
	if($_POST['page'] == 'user')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = '<button type="button" name="view_detail" class="btn btn-primary btn-sm details" id="'.$row["user_id"].'">View profile</button>';
				$data[] = $sub_array;
			}
	...
	if($_POST['page'] == 'exam_enroll')
	{
		if($_POST['action'] == 'fetch')
		{
			...
			{
				$exam->query .= '
				user_table.name_user LIKE "%'.$_POST["search"]["value"].'%" 
				';
